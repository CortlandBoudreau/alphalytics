...
{{
  ...
  "flags": ["short one-sentence flag, max 15 words each, 0-3 flags total"],
  "sentiment": "bullish",
    ...
    try:
        ...
        for i, col in enumerate(quarters):
            ...
            quarter_data.append({
                ...
                # YoY
                "yoy": {
                    "totalAssets":    pct_change(total_assets,    pv("Total Assets", prev_col)),
                    "cash":           pct_change(cash,            pv("Cash", prev_col)),
                    "totalDebt":      pct_change(total_debt,      pv("Total Debt", prev_col)),
                    "equity":         pct_change(equity,          pv("Stockholders Equity", prev_col)),
                    "retainedEarnings": pct_change(retained_earnings, pv("Retained Earnings", prev_col)),
                }
            })
...
Most recent quarter ({q['label']}):
  Total Assets: ${q['totalAssets']}M
  Current Assets: ${q['currentAssets']}M | Current Liabilities: ${q['currentLiabilities']}M
  Current Ratio: {q['currentRatio']}
  Cash: ${q['cash']}M
  Total Debt: ${q['totalDebt']}M | Net Debt: ${q['netDebt']}M
  Long Term Debt: ${q['longTermDebt']}M
  Stockholders Equity: ${q['stockholdersEquity']}M
  Retained Earnings: ${q['retainedEarnings']}M
  Debt/Equity: {q['debtToEquity']} | Debt/Assets: {q['debtToAssets']}
  Working Capital: ${q['workingCapital']}M

Return ONLY this JSON, no markdown, no backticks:
{{
  "summary": "2-3 sentence assessment of balance sheet health, liquidity, and leverage",
  "flags": ["short one-sentence flag, max 15 words, 0-3 flags total"],
  "health": "strong" | "healthy" | "moderate" | "weak" | "distressed",
  "disclaimer": "This is not financial advice. Always do your own research."
}}
"""
...
async def get_cashflow(request: Request, ticker: str, _: None = Depends(verify_token)):
    ...
    try:
        ...
        for i, col in enumerate(quarters):
            ...
            # FCF margin requires revenue — skip for now, calculate if needed
            ...
            quarter_data.append({
                ...
                # YoY
                "yoy": {
                    "operatingCashFlow": pct_change(op_cf,   pv("Operating Cash Flow", prev_col)),
                    "freeCashFlow":      pct_change(fcf,     pv("Free Cash Flow", prev_col)),
                    "capex":             pct_change(capex,   pv("Capex", prev_col)),
                    "netIncome":         pct_change(net_income, pv("Net Income", prev_col)),
                }
            })
...
Most recent quarter ({q['label']}):
  ...
  Net Income: ${q['netIncome']}M
  D&A: ${q['da']}M
  Capex: ${q['capex']}M (YoY: {q['yoy']['capex']}%)
  Stock Based Compensation: ${q['stockBasedComp']}M
  Investing Cash Flow: ${q['investingCashFlow']}M
  Financing Cash Flow: ${q['financingCashFlow']}M
  Dividends Paid: ${q['dividendsPaid']}M
  Stock Buybacks: ${q['stockBuybacks']}M
  FCF/Net Income Ratio: {q['fcfToNetIncome']}
  End Cash Position: ${q['endCashPosition']}M

Return ONLY this JSON, no markdown, no backticks:
{{
  "summary": "2-3 sentence assessment of cash generation, capital allocation, and quality of earnings",
  "flags": ["short one-sentence flag, max 15 words, 0-3 flags total"],
